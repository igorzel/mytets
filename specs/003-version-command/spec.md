...
- The existing project already uses `cobra` for command routing; the `version`
  ...
  structure.
- The build system (e.g., `go build -ldflags "-X main.Version=1.0.1"` or
  equivalent) is responsible for injecting the version string; the spec does not
  mandate a specific CI/CD toolchain.
